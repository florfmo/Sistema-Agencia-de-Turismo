...
<%@page import="Logica.ServicioTuristico, java.text.SimpleDateFormat, java.util.Date"%>
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  ...
                  <h4 class="card-title">Modificación de servicios</h4>
                  <h6 class="font-weight-light">Edite los datos del servicio</h6>
                  <form class="forms-sample" action="SvModificarServ" method="GET" id="formulario">
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="nombre" name="nombre" value="<%=serv.getNombre()%>" pattern=".{1,55}" title="Ingrese máximo 55 caracteres." required>
                    </div>
                    <div class="form-group">
                      <label for="exampleInputEmail3">Descripción breve</label>
                      <input type="text" class="form-control" id="descripcion_breve" name="descripcion_breve" value="<%=serv.getDescripcion_breve()%>" pattern=".{1,120}" title="Ingrese máximo 120 caracteres." required>
                    </div>
                    <div class="form-group">
                      <label for="exampleInputCity1">Destino</label>
                      <input type="text" class="form-control" id="destino_servicio" name="destino_servicio" value="<%=serv.getDestino_servicio()%>" pattern="[a-zA-ZÀ-ÿ\s]{1,35}" title="Sólo puede ingresar letras. Máximo 35 caracteres." required>
                    </div>
                    <div class="form-group">
                      <label for="exampleInputPassword4">Fecha</label>
                      <input type="date" class="form-control" id="fecha_servicio" name="fecha_servicio" value="<%=strDate%>" required>
                    </div>
                    <div class="form-group">
                      <label for="exampleInputCity1">Costo</label>
                      <input type="text" class="form-control" id="costo_servicio" name="costo_servicio" value="<%=serv.getCosto_servicio()%>" pattern="\d{1,8}(\.\d{1,2})?" title="Ingrese máximo 8 números (máximo dos decimales opcionales, separador decimal: ." required>
                    </div>
                    <div class="form-group">
                      <label for="exampleSelectGender">Tipo de servicio</label>
                        <select class="form-control" id="tipo_servicio" name="tipo_servicio" value="<%=serv.getTipo_servicio()%>" required>
                          <option <%if (serv.getTipo_servicio().equals("Hotel por noche/s")) { %> selected="selected" <%};%>>Hotel por noche/s</option>
                          <option <%if (serv.getTipo_servicio().equals("Alquiler de auto")) { %> selected="selected" <%};%>>Alquiler de auto</option>
                          <option <%if (serv.getTipo_servicio().equals("Pasajes de colectivo")) { %> selected="selected" <%};%>>Pasajes de colectivo</option>
                          <option <%if (serv.getTipo_servicio().equals("Pasajes de avión")) { %> selected="selected" <%};%>>Pasajes de avión</option>
                          <option <%if (serv.getTipo_servicio().equals("Pasajes de avión")) { %> selected="selected" <%};%>>Pasajes de tren</option>
                          <option <%if (serv.getTipo_servicio().equals("Excursiones")) { %> selected="selected" <%};%>>Excursiones</option>
                          <option <%if (serv.getTipo_servicio().equals("Entradas a Eventos")) { %> selected="selected" <%};%>>Entradas a Eventos</option>
                        </select>
                      <input type="hidden" name="id" value="<%=serv.getCodigo_servicio()%>">
                      </div>
                    <button type="submit" class="btn btn-primary mr-2" value="enviar" id="btn-enviar">Modificar servicio</button>
                  <%}%>
                  </form>
                </div>
              </div>
